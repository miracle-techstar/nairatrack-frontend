...
// Base API URL for your Node.js/MongoDB backend
const RAW_API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000/api/transactions';
const BASE_API_URL = RAW_API_URL.replace(/\/transactions\/?$/, '');
...
export default function NairaTrackApp() {
  ...
  const [token, setToken] = useState(() => localStorage.getItem('nairatrack_token') || '');
  ...
  const [categories, setCategories] = useState(() => {
    ...
    return saved ? JSON.parse(saved) : ['Sales', 'Rent & Utilities', 'Inventory', 'Payroll', 'Logistics'];
  });
  ...
  useEffect(() => {
    if (currentUser) {
      localStorage.setItem(`nairatrack_categories_${currentUser.id}`, JSON.stringify(categories));
    }
  ...
  useEffect(() => {
    if (currentUser) {
      localStorage.setItem(`nairatrack_budgets_${currentUser.id}`, JSON.stringify(budgets));
    }
  }, [budgets, currentUser]);
  ...
  useEffect(() => {
    if (currentUser) {
      const savedCats = localStorage.getItem(`nairatrack_categories_${currentUser.id}`);
      setCategories(savedCats ? JSON.parse(savedCats) : ['Sales', 'Rent & Utilities', 'Inventory', 'Payroll', 'Logistics']);
      
      const savedBudgets = localStorage.getItem(`nairatrack_budgets_${currentUser.id}`);
  ...
  const safeFetch = async (url, options = {}) => {
    ...
    try {
      ...
      throw new Error(`Cannot connect to server at ${url}. Please ensure your local backend is running (cd nairatrack-backend-main && npm start).`);
    }
    ...
    if (contentType && contentType.includes('application/json')) {
      ...
      if (!res.ok) {
        throw new Error(`Server returned status ${res.status}: Check if the backend is running on ${BASE_API_URL}`);
      }
    }
  ...
  const getFilteredTransactions = (items, period) => {
    ...
    return items.filter(item => {
      ...
      if (period === 'month') {
        return itemDate.getFullYear() === localDate.getFullYear() && 
  ...
  const getCurrentMonthExpenseForCategory = (catName) => {
    ...
    return transactions
      .filter(t => {
        if (t.type !== 'Expense' || t.category !== catName || !t.date) return false;
  ...
  const renderBudgetBadge = (catName) => {
    ...
    if (limit === undefined || limit === null || limit === '' || parseFloat(limit) <= 0) {
      return null;
    }
    ...
    if (ratio > 1.0) {
      return <span className="badge badge-pink" style={badgeStyle}>🔴 Exceeded</span>;
    } else if (ratio >= 0.8) {
      return (
        <span className="badge" style={{ ...badgeStyle, backgroundColor: '#fef3c7', color: '#d97706' }}>
          🟡 Warning
        </span>
      );
    } else {
      return <span className="badge badge-green" style={badgeStyle}>🟢 On Track</span>;
    }
  };

  // Computed dashboard calculations
  const dashboardFilteredTransactions = getFilteredTransactions(transactions, timeframe);
  ...
  const totalReportExpense = expenseTransactions.reduce((acc, t) => acc + parseFloat(t.amount || 0), 0);
  ...
  const userInitial = currentUser ? (currentUser.businessName || currentUser.name || 'M').charAt(0).toUpperCase() : 'M';
        ...
        <main className="main-content">
          <section className="about-section">
            <div className="card about-hero">
              ...
              <p>
                NairaTrack is a secure, multi-user expense tracking platform designed specifically for Nigerian 
                Small and Medium-sized Enterprises (SMEs). We empower business owners with password-protected accounts, 
                isolated data, cash flow tracking, custom budgets, and instant analytics.
              </p>
            </div>
            ...
            <div className="about-features">
              <div className="card about-feature-card">
                ...
                <p>Passwords hashed securely with bcrypt. Each business owner gets their own isolated private account.</p>
              </div>
              <div className="card about-feature-card">
                <div className="feature-icon">📊</div>
                <h4>Real-Time Dashboard</h4>
                <p>Monitor your revenue, expenses, and net balance with flexible timeframe filters.</p>
              </div>
              <div className="card about-feature-card">
                <div className="feature-icon">📁</div>
                <h4>Personalized Categories</h4>
                <p>Define your own operational categories and set monthly spending caps with warning alerts.</p>
              </div>
              <div className="card about-feature-card">
                <div className="feature-icon">📋</div>
                <h4>Searchable Ledger</h4>
                <p>Find, filter, and audit past transactions by description, type, and category anytime.</p>
              </div>
            </div>
          ...
          <div className="auth-card">
            ...
            {authMode === 'login' ? (
              ...
              <form onSubmit={handleRegister}>
                ...
                <button type="submit" className="btn-primary full-width" disabled={authLoading}>
                  {authLoading ? 'Creating Account...' : 'Register Business Account'}
                </button>
              </form>
            )}
          </div>
          ...
          <aside className="sidebar">
            ...
            <div className="user-profile-box">
              <div className="user-profile">
                ...
                <div>
                  <p className="user-name">{currentUser ? currentUser.businessName : 'My Business'}</p>
                  <p className="user-role">{currentUser ? currentUser.name : 'SME Owner'}</p>
                </div>
              </div>
            ...
            {activeTab === 'dashboard' && (
              <section className="view-section">
                <header className="page-header">
                  <div>
                    <h1>Dashboard</h1>
                    <p className="subtitle">Welcome back, {currentUser?.name}. Financial overview for <strong>{currentUser?.businessName}</strong>.</p>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <button onClick={openModal} className="btn-primary">+ Add Entry</button>
                  </div>
                </header>
                ...
                <div className="stats-grid">
                  <div className="card stat-card">
                    ...
                    <div className="stat-amount">₦{totalIncome.toLocaleString()}</div>
                  </div>
                  <div className="card stat-card">
                    <div className="stat-header">
                    ...
                    <div className="stat-amount">₦{totalExpense.toLocaleString()}</div>
                  </div>
                  <div className="card stat-card">
                    <div className="stat-header">
                    ...
                    <div className="stat-amount">₦{netBalance.toLocaleString()}</div>
                  </div>
                </div>
                ...
                <div className="card main-card">
                  <div className="card-header">
                    <div>
                      ...
                      <p className="subtitle">Your latest income and expenses.</p>
                    </div>
                    <button onClick={() => setActiveTab('entries')} className="btn-link">View All</button>
                  </div>
                  ...
                  {recentEntries.length === 0 ? (
                    <div className="empty-state">
                      ...
                      <button onClick={openModal} className="btn-primary" style={{ marginTop: '12px' }}>Add Entry</button>
                    </div>
                  ) : (
                      ...
                      <tbody>
                        {recentEntries.map((e) => (
                          <tr key={e._id}>
                            ...
                            <td>{e.category}</td>
                            <td className={e.type === 'Income' ? 'tag-income' : 'tag-expense'}>{e.type}</td>
            ...
            {activeTab === 'entries' && (
              <section className="view-section">
                <header className="page-header">
                  <div>
                    <h1>Entries</h1>
                    <p className="subtitle">Detailed transaction ledger for {currentUser?.businessName} ({transactions.length} total).</p>
                  </div>
                  <button onClick={openModal} className="btn-primary">+ Add Entry</button>
                </header>
                  ...
                  <div style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
                    <input 
                      ...
                      style={{ 
                        ...
                        borderRadius: '8px', border: '1px solid #e5e7eb', outline: 'none', fontSize: '14px'
                      }}
                    />
                    <select 
                      value={filterType} 
                      ...
                      style={{ 
                        ...
                        borderRadius: '8px', border: '1px solid #e5e7eb', outline: 'none', cursor: 'pointer' 
                      }}
                    >
                      ...
                      <tbody>
                        {entriesFilteredTransactions.map((e) => (
                          <tr key={e._id}>
                            ...
                            <td>₦{parseFloat(e.amount || 0).toLocaleString()}</td>
                            <td>
                              <button className="btn-danger" onClick={() => handleDeleteTransaction(e._id)}>Delete</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
            ...
            {activeTab === 'categories' && (
              <section className="view-section">
                <header className="page-header">
                  <div>
                    <h1>Categories</h1>
                    <p className="subtitle">Manage transaction classification and monthly budget targets for {currentUser?.businessName}.</p>
                  </div>
                </header>
                ...
                <div className="card main-card max-w-lg">
                  ...
                  <ul className="category-list">
                    {categories.map((c, i) => (
                      <li key={i} style={{ flexDirection: 'column', alignItems: 'stretch', gap: '8px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ fontWeight: '600' }}>{c}</span>
                          <button className="btn-danger" onClick={() => handleDeleteCategory(i)}>Remove</button>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '4px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flex: 1 }}>
                            <label style={{ fontSize: '11px', color: '#666', whiteSpace: 'nowrap' }}>Monthly Budget: ₦</label>
                            <input 
                              type="number" 
                              ...
                              style={{ 
                                ...
                                border: '1px solid #ddd', fontSize: '12px', width: '100%' 
                              }}
                            />
                          </div>
            ...
            {activeTab === 'reports' && (
              <section className="view-section">
                <header className="page-header">
                  <div>
                    <h1>Reports</h1>
                    <p className="subtitle">Financial health and category insights for {currentUser?.businessName}.</p>
                  </div>
                </header>
                ...
                <div className="card main-card max-w-lg">
                  ...
                  <div style={{ marginTop: '16px' }}>
                    {totalReportExpense === 0 ? (
                      <p style={{ color: '#888' }}>No expense data available.</p>
                    ) : (
                      Object.entries(expenseByCategory).map(([cat, amt]) => {
                        const pct = ((amt / totalReportExpense) * 100).toFixed(1);
                        const limit = budgets[cat];
                        const hasLimit = limit !== undefined && limit !== null && limit !== '' && parseFloat(limit) > 0;
                        
                        return (
                          <div className="report-item" key={cat} style={{ marginBottom: '20px' }}>
                            <div className="report-row">
                              <div>
                                <strong style={{ fontSize: '14px' }}>{cat}</strong>
                                {hasLimit && (
                                  <span style={{ fontSize: '11px', color: '#666', marginLeft: '8px' }}>
                                    (Budget: ₦{parseFloat(limit).toLocaleString()})
                                  </span>
                                )}
                              </div>
      ...
      <footer className="site-footer">
        <div className="footer-content">
          <div className="footer-brand">
            <h3>₦ NairaTrack</h3>
            <p>Helping Nigerian SMEs take control of their finances with secure, password-protected bookkeeping.</p>
          </div>
          <div className="footer-links">
            <h4>Quick Links</h4>
            <ul>
              <li><button onClick={() => setActiveTab('dashboard')}>Dashboard</button></li>
              <li><button onClick={() => setActiveTab('entries')}>Entries</button></li>
          ...
          <div className="footer-links">
            ...
            <ul>
              <li><button>Bcrypt Encrypted</button></li>
              <li><button>JWT Token Protected</button></li>
              <li><button>Isolated User Data</button></li>
            </ul>
          </div>
        </div>
        <div className="footer-bottom">
          © {new Date().getFullYear()} NairaTrack — Built for Nigerian SMEs. All rights reserved.
        </div>
      </footer>
          ...
          <div className="modal-box">
            ...
            <form onSubmit={handleAddTransaction}>
              <div className="form-group">
                ...
                <select id="entry-type" value={type} onChange={(e) => setType(e.target.value)} required>
                  <option value="Expense">Expense</option>
                  <option value="Income">Income</option>
                </select>
              </div>
              ...
              <div className="form-group">
                ...
                <input 
                  type="text" id="entry-title" value={title} 
                  onChange={(e) => setTitle(e.target.value)} 
                  placeholder="e.g. Sales Payment or Electricity Bill" required
                />
              </div>
              ...
              <div className="form-group">
                ...
                <input 
                  type="number" step="0.01" id="entry-amount" value={amount} 
              ...
              <div className="form-group">
                ...
                <select id="entry-category" value={category} onChange={(e) => setCategory(e.target.value)} required>
                  {categories.map((c) => (
                    <option key={c} value={c}>{c}</option>
                  ))}
                </select>
              ...
              <div className="form-group">
                ...
                <input 
                  type="date" id="entry-date" value={date} 
                  onChange={(e) => setDate(e.target.value)} required
                />
              </div>

              <button type="submit" className="btn-primary full-width">Save Entry</button>
            </form>
          </div>
        </div>
      )}
